...
let pointerLockRetryAt = 0;

function notePointerLockExit() {
  pointerLockRetryAt = performance.now() + 850;
}

function safelyRequestPointerLock(element) {
  if (!element || document.pointerLockElement === element || performance.now() < pointerLockRetryAt) return;
  try {
    const result = element.requestPointerLock?.();
    if (result?.catch) {
      result.catch((error) => {
        if (error?.name === 'SecurityError') notePointerLockExit();
      });
    }
  } catch (error) {
    if (error?.name === 'SecurityError') notePointerLockExit();
...
function safelyExitPointerLock(element) {
...
class ThirdPersonCameraController {
  constructor(camera, target, domElement, options = {}) {
    ...
    this._onPointerDown = (event) => {
      ...
      if (this.pointerLock && event.pointerType !== 'touch') {
        safelyRequestPointerLock(this.domElement);
  ...
  disable() {
    this.enabled = false;
    this.pointerId = null;
    safelyExitPointerLock(this.domElement);
  }
