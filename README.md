...
- The JSON includes package type, map id/name, uploaded texture/image/GIF/model metadata, changed surfaces, placed shapes, placed image/GIF planes, placed GLBs, map gravity settings, Sun/Main Light data, marker/light data, brightness/color settings, collision settings, collision box transforms, animation settings, warnings, and a note that local blob URLs are temporary.
...
- The manifest includes map settings, Sun/Main Light data, copied/duplicated objects, collision enabled/disabled state, collision box transforms, image/GIF asset info, model animation settings, object transforms, color/brightness settings, and surface texture edits.
...
- The game loads `assets/editor_maps/<mapId>/latest.json` after the built-in or blank editor map is created, including map settings, Sun/Main Light, collision edits, image/GIF planes, and animated GLB settings.
...
- Overrides can apply surface textures, surface brightness, placed GLB models, animated GLB playback, placed shapes, placed image/GIF planes, map gravity settings, Sun/Main Light, collision boxes, spawn markers, puzzle station markers, lights, transforms, and brightness.
...
- `editor/assetEditor.js`: local-only editor coordinator for admin unlock, Map Maker sessions, texture imports, image/GIF imports, GLB imports, animated GLB mixers, built-in shape placement, Sun/Main Light placement, marker/light placement, click/drag paint and place modes, editor fly/collision state, collision-box editing, selected-object keyboard editing, brightness, transforms, canvas selection, draft actions, and export actions.
...
- `editor/editorLighting.js`: shared map settings and Sun/Main Light defaults plus Three.js light object factory.
...
- `editor/editorUI.js`: DOM editor unlock box, scrollable simplified editor panel, upload lists, shape library search, map/sun panels, package type fields, selected object info, collision controls, save/export controls, and keyboard help.
- `editor/editorExport.js`: Export Editor JSON builder for new-map/update metadata, assets, placed objects, collision, animation, map settings, sun lights, markers, lights, and download helper.
...
- `editor/editorOverrides.js`: permanent static editor override loader for `assets/editor_maps/<mapId>/latest.json`, including placed shapes, image/GIF planes, GLBs, animations, collision, map settings, sun lights, markers, and lights.
- `tools/import-editor-package.mjs`: Node importer for `editor_imports/inbox/*.zip` packages, including new-map/update handling, asset copying, map gravity, sun lights, collision edits, required marker validation, and editor map index updates.
